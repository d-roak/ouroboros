...
public class Degree implements Control {
	...
	public Degree(String name) {
		...
		Degree.networkSize = Network.size();
		this.degrees = new int[Degree.networkSize];
	}
	
	//Helper, reset Stats :D
	private void resetStats() {
		for(int i = 0; i < Degree.networkSize; i++)
			this.degrees[i] = 0;
	}
	...
	//this returns false because the simulation has to continue
	public boolean execute() {
		...
		for(int node = 0; node < Network.size(); node++) {
			...
			if(current.isUp()) {
				...
				this.degrees[current.getIndex()] = nodeProtocol.degree();
			}
		}
